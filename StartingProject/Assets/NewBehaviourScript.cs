...
public class NewBehaviourScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // Spawn object on Spacebar press
        if (Input.GetKeyDown(KeyCode.Space))
        {
            GameObject spawnedObject = Instantiate(spawnPrefab, Camera.main.transform.position, Camera.main.transform.rotation);
            Rigidbody rb = spawnedObject.GetComponent<Rigidbody>();
            if (rb != null)
            {
                rb.AddForce(Camera.main.transform.forward * spawnForce);
            }
        }
